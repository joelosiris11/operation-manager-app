...
const appId = 'opsflow-app';
const USE_EMULATOR = true; // Usar Firebase Emulator
...
try {
  ...
  if (USE_EMULATOR) {
    ...
    console.log('🔥 Conectado a Firebase Emulator');
  }
} catch (error) {
    ...
    <header className="h-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b dark:border-slate-800 flex items-center justify-between px-8 shrink-0 z-20">
      ...
      <div className="flex items-center space-x-6 ml-auto">
        ...
        <div className="text-right hidden sm:block leading-tight">
          ...
          <p className="text-xs text-slate-400 font-bold font-mono">{userId?.substring(0, 16) || 'Sin Auth'}</p>
        </div>
        ...
        {Object.entries(tasksByArea).length > 0 ? Object.entries(tasksByArea).map(([area, areaTasks]) => (
          <div key={area} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {areaTasks.map(task => (
                <TaskCard key={task.id} task={task} employees={employees} setTaskToAssign={setTaskToAssign} />
              ))}
            </div>
          </div>
        )) : (
...
function TaskCard({ task, employees, setTaskToAssign }) {
  const completeTask = async () => {
    if (!db) return;
    await updateDoc(doc(db, `artifacts/${appId}/public/data/tasks`, task.id), { status: 'completed' });
  };

  return (
    <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border-2 dark:border-slate-800 shadow-sm hover:shadow-2xl hover:border-indigo-200 transition-all group relative overflow-hidden">
...
// --- Scheduling View ---
function SchedulingView({ schedule, employees, userId, onShiftChange, tasks, setTaskToAssign }) {
  return (
    <div className="max-w-6xl mx-auto py-10 animate-in fade-in">
      <h2 className="text-4xl font-black tracking-tighter uppercase mb-8">Turnos y Programación</h2>
      <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border-2 dark:border-slate-800">
        <p className="text-slate-400 text-center py-20">Vista de calendario de turnos - Próximamente</p>
      </div>
    </div>
  );
}
...
function AdminMasterTasksSection({ masterTasks }) {
  ...
  const remove = async (id) => {
    if (!db) return;
    await deleteDoc(doc(db, `artifacts/${appId}/public/data/masterTasks`, id));
  };
  return (
...
function AdminTemplatesSection({ masterTasks, taskTemplates, scheduledRoutines, areas }) {
  ...
  const addSchedule = async () => {
      if (!selectedTemplateId || !db) return;
  ...
  const removeTemplate = async (id) => {
    if (!db) return;
    await deleteDoc(doc(db, `artifacts/${appId}/public/data/taskTemplates`, id));
  };

  const removeRoutine = async (id) => {
    if (!db) return;
    await deleteDoc(doc(db, `artifacts/${appId}/public/data/scheduledRoutines`, id));
  };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {taskTemplates.map(tpl => (
                        <div key={tpl.id} className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border-2 dark:border-slate-800 flex justify-between items-center group shadow-sm">
                            ...
                            <button onClick={() => removeTemplate(tpl.id)} className="text-red-400 hover:text-red-600 p-4 rounded-2xl hover:bg-red-50 transition-all"><Trash2 size={20}/></button>
                        </div>
                    ))}
                </div>
            </div>
                    ...
                    {scheduledRoutines.map(r => {
                        ...
                        return (
                            <div key={r.id} className="bg-slate-100 dark:bg-slate-800 p-6 rounded-[2rem] flex justify-between items-center border-2 border-transparent hover:border-indigo-200 transition-all">
                                ...
                                <button onClick={() => removeRoutine(r.id)} className="text-slate-400 hover:text-red-500 p-3 hover:bg-white rounded-xl transition-all"><X size={20}/></button>
                            </div>
                        );
                    })}
                 </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {employees.map(emp => (
            <div key={emp.id} className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border-2 dark:border-slate-800 flex justify-between items-center group">
              ...
              <button onClick={() => remove(emp.id)} className="p-4 text-slate-200 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all rounded-2xl"><Trash2 size={24}/></button>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-4">
          {areas.map(a => (
            <div key={a.id} className="bg-slate-100/50 dark:bg-slate-800 px-8 py-5 rounded-[2rem] border-2 dark:border-slate-700 flex items-center space-x-4 group shadow-sm transition-all hover:border-indigo-400">
              ...
              <button onClick={() => remove(a.id)} className="text-slate-300 hover:text-red-500 transition-colors leading-none"><X size={16}/></button>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-8">
          {equipment.map(e => (
            <div key={e.id} className="bg-white dark:bg-slate-900 p-10 rounded-[3.5rem] border-2 dark:border-slate-800 flex justify-between items-center group relative overflow-hidden shadow-sm hover:border-blue-400 transition-all">
              ...
              <button onClick={() => remove(e.id)} className="p-4 text-slate-200 hover:text-red-500 relative z-10 opacity-0 group-hover:opacity-100 transition-all rounded-2xl"><Trash2 size={26}/></button>
            </div>
          ))}
        </div>
      </div>
...
function AdminLabelsSection({ labelItems }) {
    ...
    const remove = async (id) => {
      if (!db) return;
      await deleteDoc(doc(db, `artifacts/${appId}/public/data/labelItems`, id));
    };
    return (
        ...
        <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {labelItems.map(item => (
            <div key={item.id} className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border-2 dark:border-slate-800 flex justify-between items-center group shadow-sm hover:border-indigo-600 transition-colors">
              ...
              <button onClick={() => remove(item.id)} className="text-slate-200 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-colors"><X size={20}/></button>
            </div>
          ))}
        </div>
      </div>
             ...
             <div className="space-y-2">
                 {employees.map(e => (
                     <div key={e.id} className="bg-white dark:bg-slate-900 p-6 rounded-3xl border-2 dark:border-slate-800 flex justify-between">
                         ...
                         <span className="text-slate-400 text-sm">{e.roles?.[0] || 'Staff'}</span>
                     </div>
                 ))}
             </div>
        </div>
